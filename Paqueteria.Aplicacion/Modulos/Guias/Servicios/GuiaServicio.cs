...
) : IGuiaServicio
{
    public async Task<Respuesta<IReadOnlyList<GuiaRespuestaDto>>> ObtenerTodosAsync()
    {
        try
        {
            ...
            var guiasRespuesta = guias.Select(g => g.ARespuestaDto()).ToList();
            return Respuesta<IReadOnlyList<GuiaRespuestaDto>>.Exitoso(guiasRespuesta);
        }
        catch (Exception ex)
        {
            return Respuesta<IReadOnlyList<GuiaRespuestaDto>>.Error(new Error("", ex.Message));
        }
    }
    ...
    public async Task<Respuesta<GuiaRespuestaDto>> ObtenerPorIdAsync(Guid guiaId)
    {
        ...
        if (guia is null)
            return Respuesta<GuiaRespuestaDto>.Error(CodigosError.Generic.NoEncontrado);

        return Respuesta<GuiaRespuestaDto>.Exitoso(guia.ARespuestaDto());
    }
    ...
    public async Task<Respuesta<IReadOnlyList<GuiaRespuestaDto>>> ObtenerFiltroAsync(GuiaFiltroDto request)
    {
        var guias = ( await unit.Guias.ObtenerFiltroAsync(request, contextoUsuario.EmpresaId, false) )
            .Select( g => g.ARespuestaDto() ).ToList();
    ...
    public async Task<Respuesta<GuiaCreadaDto>> AgregarAsync(GuiaCrearDto dto)
    {
        try
        {
            ...
            if (direccionOrigen is null || direccionDestino is null)
                return Respuesta<GuiaCreadaDto>.Error(CodigosError.Generic.NoEncontrado);
            ...
            if (filasAfectadas <= 0)
                return Respuesta<GuiaCreadaDto>.Error(CodigosError.Generic.NoCreado);
        ...
        {
            return Respuesta<GuiaCreadaDto>.Error(new Error("", ex.Message));
        }
    }
