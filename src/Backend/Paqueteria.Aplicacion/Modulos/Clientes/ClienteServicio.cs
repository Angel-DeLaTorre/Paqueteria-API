...
public class ClienteServicio(IUnitOfWork unit, IUsuarioContextoServicio contextoUsuario) : IClienteServicio
{
    ...
    public async Task<Respuesta> AgregarDireccionAsync(Guid clienteId, DireccionDto dto)
    {
        var existe = await unit.Clientes.ExisteAsync(clienteId);
        
        if (!existe)
            return Respuesta.Error(CodigosError.Comun.NoEncontrado);

        var direccion = dto.MapeaEntidad();
        var direccionCliente = DireccionCliente.Crear
        (
            direccion,
            clienteId
        );

        await unit.Clientes.AgregarDireccionAsync(direccionCliente);
        
        var resultado = await unit.GuardarCambiosAsync();
        return resultado > 0
            ? Respuesta.Exitoso()
            : Respuesta.Error(CodigosError.Comun.NoCreado);
    }
