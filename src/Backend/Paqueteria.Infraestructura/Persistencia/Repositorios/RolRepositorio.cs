...
public class RolRepositorio (AppDbContext context) : IRolRepositorio
{
    public async Task<Rol?> ObtenerPorIdAsync(Guid id, Guid empresaId, bool includePermissions = false)
    {
    ...
    public async Task<Rol?> ObtenerPorNombreAsync(string name, Guid empresaId)
    {
        return await context.Roles
            .FirstOrDefaultAsync(r => r.Nombre == name && r.EmpresaId == empresaId);
    }
    
    public async Task<IEnumerable<Rol>> ObtenerTodosAsync(Guid empresaId)
    {
        return await context.Roles
    ...
    public async Task AgregarAsync(Rol role)
    {
        await context.Roles.AddAsync(role);
    }
    
    public void Eliminar(Rol role)
    {
        context.Roles.Remove(role);
    }
    
    public async Task AgregarPermisoAlRolAsync(Guid roleId, Guid permissionId, Guid empresaId)
    {
        ...
        if (!alreadyExists)
        {
            var rolPermiso = RolPermiso.Crear(roleId, permissionId);
            await context.RolPermiso.AddAsync(rolPermiso);
        }
    }
    
    public async Task RemoverPermisoAlRolAsync(Guid roleId, Guid permissionId, Guid empresaId)
    {
